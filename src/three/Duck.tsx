...
function DuckBody() {
  ...
  useFrame((state, delta) => {
    ...
    // idle sway keeps running always; the spin/squash springs stop
    // integrating (and stop re-writing rotation/scale/position every
    // frame) once they have actually settled back to rest
    const settled =
      Math.abs(sp.v) < 1e-4 &&
      Math.abs(sp.target - sp.x) < 1e-4 &&
      Math.abs(sq.s) < 1e-4 &&
      Math.abs(sq.v) < 1e-4
    if (!settled) {
    ...
    <group position={[-0.11, DESK_TOP + 0.46, -0.69]}>
      ...
      >
        <group ref={rig} rotation-y={-0.5}>
          ...
          <mesh geometry={geo.body} castShadow matrixAutoUpdate={false}>
          ...
          <mesh geometry={geo.wings} matrixAutoUpdate={false}>
            <meshPhysicalMaterial
              color={bodyDark}
              roughness={0.34}
              clearcoat={0.6}
          ...
          {/* bill */}
          <mesh geometry={geo.beak} matrixAutoUpdate={false}>
            <meshPhysicalMaterial
              color={P.amber}
              roughness={0.32}
              clearcoat={0.6}
          ...
          {/* glossy bead eyes */}
          <mesh geometry={geo.eyes} matrixAutoUpdate={false}>
            <meshPhysicalMaterial
              color={P.ink}
              roughness={0.06}
              clearcoat={1}
